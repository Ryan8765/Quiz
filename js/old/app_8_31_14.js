$(document).ready(function() {
	...
	var questionCorrect = function(inputAnswer) {
		...
		for (var i = 0; i < 4; i++) {
				$("label:nth-child(" + (i + 1) + ")").contents().filter(function() {
						return(this.nodeType == 3);
					}).remove();
				$("label:nth-child(" + (i + 1) + ")").append(questions[questionNumber + 1].option[i]);
				//alert(questions[questionNumber + 1].option[i])
		}
	...
	$("#input").on('click','input[type="submit"]', function(event) {
			...
			if ($("input[name='answer']").is(":checked")) {
				...
				alert('Please select an answer.....dumbass');
			}
